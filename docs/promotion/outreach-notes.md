...
tiers behind one local CLI/proxy/MCP server. It starts without an API key and can
use optional user-owned free-tier keys for more models and capacity.

Repo: https://github.com/0xzr/freellmpool
Docs: https://0xzr.github.io/freellmpool/

Current catalog: 19 providers, 235 enabled chat routes, 355 cataloged chat
models.
...
Vi que has cubierto herramientas de IA y APIs gratuitas. Publique
`freellmpool`, una herramienta MIT/open-source en Python para agrupar free tiers
de proveedores LLM detras de una sola interfaz local.

Repo: https://github.com/0xzr/freellmpool
Docs: https://0xzr.github.io/freellmpool/

Funciona como CLI, proxy local compatible con OpenAI, libreria de Python y
servidor MCP. Puede responder sin API keys para empezar, y tambien permite sumar
keys gratuitas propias de Groq, Cerebras, Gemini, Mistral, OpenRouter, NVIDIA,
etc.

Catalogo actual: 19 proveedores, 235 rutas chat habilitadas, 355 modelos chat
catalogados.
...
Si te sirve para un video o post, feliz de responder preguntas.
...
Anthropic-compatible path, and MCP server. It starts without an API key, supports
optional user-owned free-tier keys, and currently catalogs 19 providers with 235
enabled chat routes. Useful for coding-agent side tasks, docs, triage, and
scripts where free-tier models are good enough.
